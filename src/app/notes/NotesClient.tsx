...
export default function NotesClient({ notes }: { notes: PublicNoteSummary[] }) {
        ...
        {filteredNotes.map((note) => (
          ...
          >
            ...
            <div className={styles.bentoTags}>
              {note.tags.map((tag) => <span key={tag} className={styles.bentoTag}>{tag}</span>)}
            </div>
            <div className={styles.bentoFooter}>
              <span className={styles.bentoDate}>{note.date}</span>
              <Link href={`/notes/${note.id}`} className={styles.bentoLink}>查看详情 <span>→</span></Link>
            </div>
          </div>
        ))}
