...
export const allNotes: Note[] = [
  {
    ...
    description: '基于 Cron 的 macOS 桌面文件自动归档系统，支持 17 个关键词→项目映射、iCloud 文件锁绕行、重名冲突处理，每两天自动将散落文件归入对应目录。',
    detail: `## 为什么需要自动归档

大爷的 Mac 桌面上长期散落着各种工作文件——文旅项目材料、社保方案、装修报价……手动整理效率低，而且容易遗漏。之前 AI 助手可以帮忙整理，但每次都要重新判断归属，重复劳动。
...
所有文件归档到 ~/Documents/ 下，按编号前缀分类：

- **00 收件箱** — 临时文件、整理预演、总索引
- **01 工作资料** — 工作文档（按年份/项目分层）
- **05 安装包与驱动** — .dmg .pkg .app 安装文件
- **06 项目代码** — Git 仓库、代码项目
- **98 系统与软件遗留配置** — 旧系统配置文件
- **99 历史归档** — 不再活跃的旧项目

桌面作为"临时托盘"，仅保留四个文件夹：00-今日临时、01-本周处理、02-常用快捷方式、99-历史归档。
...
主索引文件：\`~/Documents/00-收件箱/📌文件整理总索引-START-HERE.md\`。以后无论是人找文件还是 AI 整理文件，都从这里开始。`,
